...
/* ═══════════════════════════════════════════
   CONTENT DATA — edit in one place
   ═══════════════════════════════════════════ */
...
const ICON_MAP = { TrendingUp, GitMerge, Building2, Cpu, HeartPulse, Shield, Factory };
...
function SectionWrap({ id, dark, children, className = "" }) {
  return (
    <section
      id={id}
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-navy">
      ...
      <div className="absolute inset-0">
        {/* Radial gradient */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_30%_20%,rgba(0,131,143,0.12)_0%,transparent_60%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_70%_80%,rgba(212,168,67,0.06)_0%,transparent_50%)]" />
        {/* Grid pattern */}
        <div
      ...
      <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-6">
        {SECTORS.items.map((s, i) => (
          <FadeUp key={i} delay={i * 0.1}>
            ...
            >
              <div className="w-11 h-11 rounded-lg bg-teal/8 flex items-center justify-center mb-4">
...
function HeadshotPlaceholder({ initials }) {
  return (
        ...
        {TEAM.principals.map((p, i) => (
          <FadeUp key={i} delay={i * 0.12}>
            <div className="text-center">
              <HeadshotPlaceholder initials={p.initials} />
...
function ServiceItem({ item, isOpen, toggle, index }) {
  return (
      ...
      <div className="border-b border-navy-mid">
        ...
        <AnimatePresence initial={false}>
          {isOpen && (
            ...
            >
              <p className="pl-17 pb-6 text-gray-400 text-sm md:text-base leading-relaxed max-w-3xl" style={{ paddingLeft: "4.25rem" }}>
                {item.desc}
              </p>
            </motion.div>
          )}
    ...
    <footer className="bg-[#060D17] py-10 px-5 sm:px-8 border-t border-navy-mid">
      <div className="mx-auto max-w-6xl flex flex-col md:flex-row items-center justify-between gap-4">
        <p className="text-gray-500 text-xs text-center md:text-left">
          © {new Date().getFullYear()} {SITE.name}. A wholly owned subsidiary of the Toronto Business Development
          Centre.
        </p>
